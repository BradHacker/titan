...
func (aq *ActionQuery) QueryInstruction() *InstructionQuery {
...
func (aq *ActionQuery) WithInstruction(opts ...func(*InstructionQuery)) *ActionQuery {
	query := &InstructionQuery{config: aq.config}
...
//
//	client.Action.Query().
//		GroupBy(action.FieldActionType).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
//
...
func (aq *ActionQuery) sqlAll(ctx context.Context) ([]*Action, error) {
	...
	if query := aq.withInstruction; query != nil {
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "instruction_action" returned %v`, n.ID)
			}
			for i := range nodes {
